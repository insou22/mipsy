    ...
    pub fn message(&self) -> String {
        match self {
            ...
            Error::InstructionInDataSegment => {
                let message_1 = "cannot put instruction into data segment"
                    .bright_red()
                    .bold();
    ...
    pub fn tips(&self) -> Vec<String> {
        match self {
            ...
            Error::DataInTextSegment { directive_type } => {
                let data = ".data".bold();
                let tip = format!(
                    "you may want to insert a `{}` directive before your `{}{}`\n",
                    data,
                    ".".bold(),
                    directive_type.to_string().bold()
                );
            ...
            Error::InstructionInDataSegment => {
                let text = ".text".bold();
                let tip = format!(
                    "you may want to insert a `{}` directive before your instruction\n",
                    text
                );
