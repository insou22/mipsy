...
use crate::{
    error::{compiler, InternalError, MipsyInternalResult},
    util::Safe,
    InstSet, MipsyResult, MpProgram,
};
...
use self::{
    breakpoints::{Point, Watchpoint, WatchpointTarget},
    extra::move_labels,
};
...
impl Binary {
    pub fn get_label(&self, label: &str) -> MipsyInternalResult<u32> {
        if let Some(&addr) = self.labels.get(label) {
            ...
            Err(InternalError::Compiler(compiler::Error::UnresolvedLabel {
        ...
        V: Point,
    {
        ...
        if hashmap.values().any(|bp| bp.get_id() == id) {
            ...
            let mut ids = hashmap.values().map(|bp| bp.get_id()).collect::<Vec<_>>();
