...
use mipsy_parser::{
    parse_argument, MpArgument, MpImmediate, MpInstruction, MpNumber, MpRegister,
...
impl InstMetadata {
    ...
    pub fn desc_short(&self) -> Option<&str> {
        self.desc_short.as_ref().map(String::as_str)
    }

    pub fn desc_long(&self) -> Option<&str> {
        self.desc_long.as_ref().map(String::as_str)
    }
}
        ...
        for (arg_type, &arg) in self.compile.format.iter().zip(args.iter()) {
            arg_bits.push(match arg_type {
                ...
                ArgumentType::J => match arg {
                    MpArgument::Number(num) => match num {
                        MpNumber::Immediate(imm) => match imm {
                            MpImmediate::LabelReference(label) => program.get_label(label)?,
                            _ => unreachable!(),
                        },
                        _ => unreachable!(),
                    },
                    ...
                    MpImmediate::LabelReference(ref label) => {
                        ...
                        if self.compile.relative_label && last {
                            let current_inst_addr =
                                (program.text.len() + self.expand.len() - 1) as u32 * 4 + TEXT_BOT;
            ...
            match arg_type {
                ...
                ArgumentType::Off32Rs | ArgumentType::Off32Rt => {
                    let reg = match arg {
                        ...
                        | MpArgument::Register(MpRegister::Offset(_, id)) => {
