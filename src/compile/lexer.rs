...
pub fn tokenise(program: &str) -> RSpimResult<Vec<Token>> {
    match tokenise1(program) {
        Ok(v) => Ok(v),
        Err(msg) => cerr!(CompileError::String(msg)),
    }
}

pub fn tokenise1(program: &str) -> Result<Vec<Token>, String> {
